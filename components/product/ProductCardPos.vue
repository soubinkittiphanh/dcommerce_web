<template>
  <div>
    <v-hover v-slot:default="{ hover }" open-delay="100">
      <v-card :elevation="hover ? 16 : 4" hover outlined
        class="ma-0 text-center cursor-pointer product-card d-flex flex-column position-relative overflow-hidden"
        height="220" :disabled="(!product.isActive || stock <= 0) && product.validateStockOnSale == 1
          " :class="{
            'product-disabled':
        ...
        <!-- Background Image -->
        <div class="product-background ma-0" :class="{
          'grayscale-filter':
            (!product.isActive || stock <= 0) &&
            product.validateStockOnSale == 1,
        }" :style="!imageError
        ...
        <div class="content-overlay d-flex flex-column justify-space-between fill-height">
          ...
          <div class="top-section pa-2">
            ...
            <v-chip v-if="getCustomerGradePrice(product)" x-small :color="getGradeColor(selectedCustomer?.grade)"
              text-color="white" class="grade-price-badge">
              <v-icon x-small left>mdi-account-star</v-icon>
              Grade {{ selectedCustomer?.grade }}
            </v-chip>

            <!-- Price List Badge (if available) -->
            <v-chip v-else-if="hasAvailablePriceLists(product)" x-small color="purple" text-color="white"
            ...
            <div class="price-actions-enhanced">
              ...
              <div class="price-section-enhanced">
                ...
                <div v-if="getCustomerGradePrice(product)" class="text-center">
                  ...
                  <div class="caption text-decoration-line-through price-text enhanced-text" style="opacity: 0.85">
                    {{ formatNumber(product.localPrice || product.pro_price) }} {{
                      findCurrency(product.saleCurrencyId).code }}
                  </div>
                ...
                <div v-else>
                  ...
                  <div v-if="hasAvailablePriceLists(product)" class="caption price-text enhanced-text"
                    style="opacity: 0.85">
                ...
                " class="d-inline-block ml-2">
                  <v-btn @click.stop="handlePriceSelection" color="primary" outlined x-small icon :disabled="(!product.isActive || stock <= 0) &&
                    product.validateStockOnSale == 1
    ...
    handleQuickAdd(isGift = false) {
      ...
      if (this.$toast) {
        ...
        if (limit && limit > 0) {
          ...
          this.$toast.success(
            `${this.productName} added to cart. ${remaining > 0 ? `${remaining} more allowed` : 'Limit reached'
            }`,
