    ...
    <v-card class="pa-4">
      ...
      <v-card-text>
        ...
        <div>
          ...
          <v-row no-gutters>
            <v-chip
              v-for="menu in form.menuLines"
              :key="menu.id"
              class="ma-2"
              color="warning"
              variant="outlined"
              @click="removeMenuLine(menu)"
            >
              {{ menu.id }} - {{ menu.name }}
            </v-chip>
          </v-row>
...
export default {
  ...
  methods: {
    ...
    addMenuLine() {
      ...
      const newMenuLine = this.menuLineList.find(
        (el) => el.id == this.menuLineSelected
      )
      console.log(
        `Authority ${newMenuLine.id} len ${this.form.menuLines.length}`
      )
      ...
      if (this.form.menuLines.length == 0) {
        ...
        const menu = this.form.menuLines.find(
          (el) => el.id == this.menuLineSelected
        )
    ...
    async commitRecord() {
      if (this.$refs.form.validate() && !this.isloading) {
        ...
        let api = this.isCreate
          ? 'api/menuHeader/create'
          : `api/menuHeader/update/${this.recordId}`
        console.log('API => ', api)
        if (this.isCreate) {
          await this.$axios
            .post(api, this.form)
    ...
    async loadEntry() {
      ...
      if (this.recordId && !this.isCreate) {
        await this.$axios
          .get(`api/menuHeader/find/${this.recordId}`)
          .then((response) => {
