<template>
  <v-container fluid class="pa-0 fill-height">
    ...
    <v-dialog v-model="showPriceOverrideDialog" max-width="500" persistent>
      <v-card>
        ...
        <v-card-text class="pt-4">
          ...
          <v-alert v-if="getCustomerGradePrice(selectedProduct)" color="success" text dense class="mb-3">
          ...
          " type="info" dense text class="mb-3">
          ...
          <v-textarea v-if="
            customPriceInput ||
            selectedPriceListId ||
            !getCustomerGradePrice(selectedProduct)
          " v-model="priceOverrideReason" label="Reason (optional)" outlined dense rows="2" counter="200"
    ...
    <v-row no-gutters class="fill-height pos-main-container">
        ...
        <div class="category-container">
          ...
          <div class="category-content">
            ...
            <v-list dense class="pa-0">
              <v-list-item @click="categoryFilter = null" :class="{
                'v-list-item--active primary--text': !categoryFilter,
              }" class="category-item">
                <v-list-item-icon class="mr-3">
                  <v-icon :color="!categoryFilter ? 'primary' : 'grey'">mdi-view-grid</v-icon>
                </v-list-item-icon>
                <v-list-item-content>
                  <v-list-item-title>All Products</v-list-item-title>
                  <v-list-item-subtitle>{{ products.length }} items</v-list-item-subtitle>
                </v-list-item-content>
              </v-list-item>
              ...
              <!-- Dynamic Categories -->
              <v-list-item v-for="category in categoryOptions" :key="category.categ_id"
                @click="categoryFilter = category.categ_id" :class="{
                  'v-list-item--active primary--text':
                    categoryFilter === category.categ_id,
                }" class="category-item">
                <v-list-item-icon class="mr-3">
                  <v-icon :color="getCategoryColor(category.categ_name)">
                    {{ getCategoryIcon(category.categ_name) }}
                  </v-icon>
                </v-list-item-icon>
                <v-list-item-content>
                  <v-list-item-title>{{
                ...
                <v-list-item-action v-if="categoryFilter === category.categ_id">
      ...
      <v-col cols="6" class="products-panel">
        <div class="products-container">
          ...
          <v-card-title class="products-header secondary white--text pa-3">
            ...
            <v-chip v-if="selectedCustomer && selectedCustomer.grade" :color="getGradeColor(selectedCustomer.grade)"
              class="mr-2">
          ...
          <div class="products-search-section">
            <v-card-text class="pa-3 pb-1">
              <v-row>
                <v-col cols="8">
                  <v-text-field v-model="searchQuery" prepend-inner-icon="mdi-magnify" label="Search products..."
                    outlined dense clearable hide-details />
                </v-col>
                <v-col cols="4">
                  <v-btn @click="fetchProducts" :loading="loading" color="primary" block>
          ...
          <div class="products-content">
            ...
            <div v-else class="pa-3">
              <v-row v-if="filteredProducts.length > 0">
                <v-col v-for="product in filteredProducts" :key="product.id" cols="4" sm="4" md="3" lg="3" xl="2"
                  class="pa-2">
                  <v-card elevation="2" hover class="product-card-enhanced d-flex flex-column overflow-hidden"
                    :disabled="(!product.isActive || product.stock_count <= 0) && product.validateStockOnSale" :class="{
                    ...
                    <div class="product-image-container">
                      <v-img v-if="product.pro_image_path" :src="`${$axios.defaults.baseURL}/${product.pro_image_path}`"
                        height="140" cover transition="fade-transition" class="product-image grey lighten-4">
                      ...
                      <div v-else class="product-image-placeholder fill-height">
                        <v-icon size="48" color="grey lighten-3">{{ getCategoryIcon(product.categ_name) }}</v-icon>
                      </div>

                      <!-- Overlay Badges -->
                      <div class="badge-overlays">
                        <!-- Promotion indicator -->
                        <v-tooltip bottom v-if="isProductInPromotion(product)">
                          <template v-slot:activator="{ on, attrs }">
                            <v-chip v-bind="attrs" v-on="on" x-small color="success" class="promo-badge-chip px-1">
                        ...
                        <v-chip :color="getStockColor(product.stock_count)" text-color="white" x-small
                          class="stock-badge-chip font-weight-bold">
                          {{ product.stock_count }}
                        </v-chip>
                      </div>

                      <!-- Customer Grade Price Overlay (Top Left) -->
                      <v-chip v-if="getCustomerGradePrice(product)" x-small
                        :color="getGradeColor(selectedCustomer.grade)" text-color="white"
                        class="grade-badge-overlay font-weight-bold">
                    ...
                    <div class="product-info-content pa-2 flex-grow-1 d-flex flex-column">
                      <div class="product-name-text mb-1">{{ product.pro_name }}</div>
                      ...
                      <div class="mt-auto">
                        ...
                        <!-- Add Button & Price Selector -->
                        <div class="d-flex align-center mt-2 justify-space-between">
                          <v-spacer></v-spacer>
                          <v-btn v-if="hasAvailablePriceLists(product) || !getCustomerGradePrice(product)"
                            @click.stop="openPriceSelector(product)" color="purple" x-small icon class="mr-1">
                            <v-icon size="18">mdi-tag-multiple</v-icon>
                          </v-btn>

                          <v-btn color="primary" x-small fab elevation="2" @click.stop="addToCart(product)">
                    ...
                    <!-- Inactive Overlay -->
                    <div v-if="!product.isActive" class="inactive-overlay d-flex align-center justify-center">
                      <v-chip color="error" small class="font-weight-bold">INACTIVE</v-chip>
                    </div>
                  </v-card>
                </v-col>
              </v-row>
      ...
      <v-col cols="4" class="cart-panel">
        <div class="cart-container">
          <!-- Cart Header (FIXED) -->
          <v-card-title class="cart-header white--text primary">
          ...
          <div class="cart-footer">
            ...
            <div v-if="cart.length > 0" class="cart-actions">
              <v-row dense no-gutters class="mb-1">
                ...
                <v-col cols="6" class="pl-1">
                  <v-btn @click="saveTicket" color="primary" block small :loading="savingTicket">
              ...
              <v-row dense no-gutters class="mb-1">
                <v-col cols="6" class="pr-1">
                  <v-btn @click="processPayment" color="primary" block>
                    <v-icon small class="mr-1">mdi-credit-card</v-icon>
                    Payment
                  </v-btn>
                </v-col>
                <v-col cols="6" class="pl-1">
                  <v-btn @click="printCustomerReceipt" color="primary" block :disabled="!currentTicket">
            ...
            <!-- Always visible Close button (when in dialog mode) -->
            <v-btn v-if="dialogMode" @click="closeScreen" color="error" block small outlined class="mt-2">
...
export default {
  ...
  data() {
    return {
      ...
      // Search and filters
      searchQuery: '',
      categoryFilter: null, // Changed from '' to null for better filtering
      categoryOptions: [],
  ...
  computed: {
    ...
    effectivePriceInDialog() {
      ...
      const customerGradePrice = this.getCustomerGradePrice(
  ...
  methods: {
    ...
    getSelectedCategoryName() {
      ...
      const category = this.categoryOptions.find(
        (c) => c.categ_id === this.categoryFilter
      )
    ...
    getProductTax(product) {
      ...
      // 1. If product has tax object attached from backend (via include=tax)
      if (product.tax) {
        return product.tax
      }

      // 2. If product has specific taxId
      if (product.taxId) {
      ...
      // 3. If taxId is explicitly null or 0, it means NO TAX (don't use default)
      if (product.taxId === null || product.taxId === 0) {
        return null
      }

      // 4. Final fallback only for legacy or missing data
      return this.defaultTax
    },
        ...
        let ticketData = {
          ...
          ticketLines: this.cart.map((item) => {
            ...
            return {
              ...
              priceOverridden:
                item.priceOverridden || item.isCustomerGradePrice || false,
              priceOverrideReason:
    ...
    applyPriceSelection() {
      ...
      const isCustomerGrade =
        this.getCustomerGradePrice(this.selectedProduct) &&
        !this.customPriceInput &&
        !this.selectedPriceListId
      ...
      let message = `${this.selectedProduct.pro_name
        } added with ${this.formatPrice(effectivePrice)}`
      if (isCustomerGrade) {
...
.product-card-enhanced:hover:not(.product-disabled) {
