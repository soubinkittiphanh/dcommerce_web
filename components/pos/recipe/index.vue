  ...
  <div class="recipe-management">
    ...
    <div class="page-header">
      <div class="header-content primary rounded pa-4 mr-4">
        <h1 class="page-title">
          <v-icon large color="white" class="mr-3">mdi-book-open-variant</v-icon>
          Recipe Management
        </h1>
        <p class="page-subtitle">Manage product recipes and ingredients</p>
      </div>
        ...
        <v-form ref="recipeForm" v-model="formValid" lazy-validation>
          <v-card-text>
            ...
            <v-row class="mb-4">
              <v-col cols="12">
                <v-text-field
                  v-model="currentRecipe.recipeName"
                  label="Recipe Name"
            ...
            <v-row class="mb-4">
              <v-col cols="12">
                <v-autocomplete
                  v-model="currentRecipe.productId"
                  :items="finishedProducts"
                  item-text="pro_name"
                  item-value="id"
                ...
                <v-alert type="info" outlined>
                  <div class="d-flex align-center">
                    ...
                    <div class="text-right">
                      ...
                      <div><strong>Current Cost:</strong> {{ formatCurrency(selectedProduct.pro_cost_price || 0) }}</div>
                    </div>
                  </div>
                </v-alert>
              </v-col>
                ...
                >
                  <template v-slot:item.name="{ item, index }">
                    <v-text-field
                      ...
                      prepend-inner-icon="mdi-tag"
                      @input="generateIngredientName(index)"
                    ></v-text-field>
                  </template>
                  ...
                  <template v-slot:item.ingredientId="{ item, index }">
                    <v-autocomplete
                      ...
                      :items="getAvailableIngredients(index)"
                      item-text="pro_name"
                      item-value="id"
                      ...
                      outlined
                      clearable
                      :search-input.sync="ingredientSearchInputs[index]"
                      placeholder="Search ingredients..."
                  ...
                  <template v-slot:item.totalCost="{ item }">
                    <strong class="primary--text">{{ formatCurrency(getIngredientPrice(item.ingredientId) * (item.quantity || 0)) }}</strong>
                  </template>
          ...
          <v-card-actions>
            ...
            <v-btn
              color="primary"
              :loading="saving"
              :disabled="!formValid || currentRecipe.ingredients.length === 0"
              @click="saveRecipe"
            >
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-row class="mb-4">
            <v-col cols="12">
              <v-text-field
                v-model="bulkRecipes.recipeName"
                label="Recipe Name"
                :rules="[rules.required]"
                outlined
                prepend-inner-icon="mdi-tag"
          ...
          >
            <template v-slot:item.name="{ item, index }">
              <v-text-field
                ...
                outlined
                placeholder="Custom ingredient name..."
                @input="generateBulkIngredientName(index)"
              ></v-text-field>
            </template>
            ...
            <template v-slot:item.quantity="{ item, index }">
              <v-text-field
                ...
                dense
                placeholder="0.00"
                @input="updateBulkRecipe(index, 'quantity', item.quantity)"
              ></v-text-field>
            </template>
