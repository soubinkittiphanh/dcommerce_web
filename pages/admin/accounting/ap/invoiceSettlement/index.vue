<template>
  <div>
    ...
    <div class="d-flex align-center mb-6">
      ...
      <div>
        <h1 class="text-h5 font-weight-bold mb-1 primary--text">ການຊຳລະໜີ້ (AP Settlement)</h1>
        <p class="text-caption grey--text text--darken-1 mb-0">AP Settlement & Payment Tracking</p>
      </div>
      <v-spacer />
      ...
      <v-btn color="primary" depressed large @click="openDialog()" class="rounded-lg font-weight-bold">
      ...
      <v-card>
        ...
        <v-card-text class="pa-4">
          ...
          <div v-else-if="settlementDetails">
            ...
            <!-- Documents Section -->
            <div v-if="settlementDetails.documents && settlementDetails.documents.length > 0" class="mt-4">
              <v-divider class="mb-3" />
              <h4 class="mb-2">ເອກະສານຂັດຕິດ (Attached Documents)</h4>
              <v-row dense class="pa-1">
                <v-col
                  v-for="(doc, idx) in settlementDetails.documents"
                  :key="idx"
                  cols="12"
                  sm="6"
                >
    ...
    async saveSettlement(formData) {
      try {
        ...
        if (formData.id) {
          ...
          response = await this.$axios.post('/api/ap-invoices-settlement', payload)
